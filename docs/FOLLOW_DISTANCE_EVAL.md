...
Use this metric when testing whether the RoboMaster holds a person at the
`CMD_FOLLOW` target distance of 1.5 m.
...
```text
hold_rate = samples with abs(depth_m - 1.5) <= 0.15 / valid depth samples
...
- `too_close`: worst distance inside the 1.5 m target.
- `too_far`: worst distance outside the 1.5 m target.
